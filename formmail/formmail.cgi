...
# HE-MOD-F                                                                   #
## This version modified by Host Europe Internet Ltd
## Accordingly, parts are copyright 2002 Host Europe Internet Ltd
## Host Europe sub-version: $Revision: 192.8 $
...
@referers = ('cespage.com','zunecardr.com','cesware.com','cesnote.com','comentsys.com');
...
#
# Any email addresses must match *exactly*; any domains (ie, no '@' sign)
# will match anything@thatdomain
#
# Just stick any extra recipients in the brackets like this:
# @recipients = &fill_recipients(@referers, 'my@email.address','som.other.com');
#
...
# 'Flash' workaround (for those obscure circumstances when the
# browser sends no referrer, but should!
#
# LEAVE THIS SET TO '0' unless you know you need it (you may if you're using
#  current versions of Shockwave Flash to submit to the form), in which case
#  you can set it to '1'
my $null_referrer_allowed=0;

# Usability hack: Automatically import $ENV{HTTP_HOST} into @referers, which
# can save shoving it full of domain names for sites with multiple
# ServerAliases.
# Note: this could draw more attention from blind-firing formmail finders (ie,
# those which have an IP to look at but not a domain).
# Set to zero if you don't want this, but it should largely be safe.
# Note: this does not repopulate or append to @recipients
#
my $http_host_import=1;

# Done                                                                       #
##############################################################################

push @referers, $ENV{HTTP_HOST} if $http_host_import;
...
sub check_url {
    ...
    if ($ENV{'HTTP_REFERER'}) {
        foreach $referer (@referers) {
            if ($ENV{'HTTP_REFERER'} =~ m|^https?://([^/\@]*\@)?([\w\.\-]*\.)?\Q$referer\E/|i) {
                $check_referer = 1;
                last;
            }
        }
				...
				# was set =1, changed for 'security' reasons.
        $check_referer = $null_referrer_allowed;
    }

    # If the HTTP_REFERER was invalid, send back an error.                   #
    if ($check_referer != 1) { 
			warn "Empty or invalid referrer '$ENV{HTTP_REFERER}', ".
...
sub send_mail {
    ...
    # Valid-email-not-required hack
    my ($from_email,$from_name)=(check_email($Config{'email'}))?
        ($Config{'email'},$Config{'realname'}):
        ("webmaster\@$ENV{HTTP_HOST}",
            "$ENV{HTTP_HOST} sending on behalf of '$Config{'realname'}'");

    # Open The Mail Program
    #Anti-AOL hack:
    open(MAIL,"|$mailprog -f $from_email");
...
sub fill_recipients {
    ...
    foreach $domain (@domains) {
			my $namepart;
			if ($domain=~/\@/) {
				($namepart, $domain)=split('\@',$domain,2);
				$namepart="\Q$namepart\E";
			} else {
				$namepart='[\w\-\.]+';
			}
			if ($domain =~ /^\d+\.\d+\.\d+\.\d+$/) {
				...
				$domain =~ s/([\.\-])/\\$1/g;
				$domain=~s/(\w)/\[\u$1\l$1\]/g;
			}
			push(@return_recips,"$namepart\@$domain");
    }
